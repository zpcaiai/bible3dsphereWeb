...
12 个 Batch 的代码、数据库、用户工作流、内容治理、情境运行时和证据控制台已经接入现有 split-repo 产品。真实 Chrome 自动与本地全栈验收、完整 230 文件 PostGIS 历史链、0238–0241 回滚/重放和受保护 Vercel Preview 已执行；生产前后端默认开关保持关闭，Preview 仅打开前端开关。它们仍不能对真实用户开放：神学、牧养、儿童安全、隐私/权利、内容质量、人工无障碍与最终发布签署没有授权人证据，独立后端 staging 也未部署。
...
| 数据库 | `0238` 基础表/RLS；`0239` 并发、精确哈希、角色审核、发布范围；`0240` 67 个 review-only 资源；`0241` 五角色证明与职责分离 |
...
| 本地 Chrome Playwright + axe | 6/6 passed：桌面键盘/最小化数据、390×844 与 320×568 reflow/44px 触控/减弱动态、admin fail-closed、400% 缩放/语义地标；0 axe violations | 0 |
| 本地真实全栈 Chrome | 默认账号登录 → Vite 同源代理 → FastAPI → PostGIS，HTTP 201；active learner_context、90 天保留与审计记录均入库 | 0 |
| 完整历史 PostGIS 迁移链 | PostgreSQL 16 / PostGIS 3.6.1 / vector 0.8.6；230/230 strict forward；幂等重跑 0；0238–0241 回滚并重放；67 content / 0 published | 0 |
| 受保护 Vercel Preview | `dpl_62R11CqCgXN5FZAJ13BRF3dh3Hd2`，target=preview，READY，授权 HTTP 200、未授权 302、`noindex,nofollow`；前端开关打开；未提升 production | 0 |
...
| 前端全量（修复后） | 145 files / 715 tests passed | 0 |
| 启动包预算门禁 | 734,176 / 900,000 bytes；3D/PDF/地图/语音等 specialist chunk 均未 eager/precache | 0 |
| 当前新增功能 ESLint scope | 0 errors / 0 warnings | 0 |
...
3. 数据库按 `0241`、`0240`、`0239`、`0238` 的 `.down.sql` 逆序运行。0240 只删除 Skill 种子；用户记录需先按数据权利流程导出/处置。
...
- 本地真实 Chrome 自动化与 authenticated FastAPI/PostGIS E2E 已通过并保存 JSON/截图；Vercel Preview 已 READY 且授权 HTTP 200。VoiceOver、人工 200%/400% 缩放、iPhone/Android 实机与具名人工签署仍缺失。
- 前端 Preview 已部署并打开前端开关；其 `/api` rewrite 仍指向生产 Hugging Face，独立后端 staging 因未登录而未部署，所以没有完整的 authenticated staging smoke。
- 自动事故/回滚演练已通过，但 limited rollout 未签发，事故 owner、回滚 owner 和最终发布人均未签署。
- 完整 fresh-database 历史链已在真实 PostGIS 环境 230/230 通过；仓库只有 0238–0241 提供 down SQL，因此更早历史迁移只验证 forward，不声称具备全链 rollback。
