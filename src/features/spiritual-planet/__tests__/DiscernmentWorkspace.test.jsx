...
describe('DiscernmentWorkspace', () => {
  ...
  it('exposes Batches 07-10 with consent, evidence and certification boundaries', async () => {
    render(<DiscernmentWorkspace />)
    expect(screen.getByText('DISCERNMENT · BATCH 01—10')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '成长轨迹' }))
    expect(await screen.findByText('成圣成长数字孪生')).toBeTruthy()
    expect(screen.getByText(/不生成“属灵成熟度总分”/)).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '协作授权' }))
    expect(await screen.findByText('教会与同行协作中心')).toBeTruthy()
    expect(screen.getByText(/最小必要披露/)).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '神学证据' }))
    expect(await screen.findByText('神学知识与证据图谱')).toBeTruthy()
    expect(screen.getByText(/不臆造书名、页码或引文/)).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '认证状态' }))
    expect(await screen.findByText('生产认证与持续复核')).toBeTruthy()
    expect(screen.getByText(/本地构建和测试不构成生产认证/)).toBeTruthy()
