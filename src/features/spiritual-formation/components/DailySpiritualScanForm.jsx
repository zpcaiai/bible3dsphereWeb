  ...
  return (
    <section className="sf-section">
      ...
      <div className="sf-form-grid">
        ...
        <label>{T('随后发生的行为', 'Behavior that followed')}<span style={{ position: 'relative', display: 'block' }}><textarea style={{ paddingRight: 96 }} value={form.behaviorDescription} onChange={(e) => update('behaviorDescription', e.target.value)} placeholder={T('你做了、逃避了、说了、点击了、隐藏了或追求了什么？', 'What did you do, avoid, say, click, hide, or pursue?')} /><SuggestMenu accent="#8c8cff" options={displayPattern.commonSymptoms} value={form.behaviorDescription} onChange={(v) => update('behaviorDescription', v)} /></span></label>
      </div>
      ...
      <div className="sf-form-grid">
        <label>{T('核心谎言', 'Core lie')}<span style={{ position: 'relative', display: 'block' }}><textarea style={{ paddingRight: 96 }} value={form.coreLie} onChange={(e) => update('coreLie', e.target.value)} placeholder={displayLies[0]} /><SuggestMenu accent="#8c8cff" options={displayLies} value={form.coreLie} onChange={(v) => update('coreLie', v)} /></span></label>
        <label>{T('福音真理', 'Gospel truth')}<span style={{ position: 'relative', display: 'block' }}><textarea style={{ paddingRight: 96 }} value={form.gospelTruth} onChange={(e) => update('gospelTruth', e.target.value)} placeholder={displayTruths[0]} /><SuggestMenu accent="#8c8cff" options={displayTruths} value={form.gospelTruth} onChange={(v) => update('gospelTruth', v)} /></span></label>
