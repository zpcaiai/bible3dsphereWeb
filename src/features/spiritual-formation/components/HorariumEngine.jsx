...
export default function HorariumEngine({ userId, initialTodayLog, history = [], onSave }) {
  ...
  const saveLabel = SAVE_LABELS[saveState] || '保存今日'
  ...
  return (
    <section className="sf-section">
      <div className="sf-section-heading holy-life-heading">
        <div>
          <h2>定时祷告 · Horarium</h2>
          <p>威廉·劳《敬虔与圣洁生活的严肃呼召》的固定祷告时辰：让一天在固定时刻回到神面前。</p>
        </div>
      ...
      <div className="holy-life-summary">
        ...
        <article className="sf-card">
          ...
          <p>最长 {streak.longest} 天 · 累计 {streak.total} 天</p>
        </article>
        <article className="sf-card">
          <h3>今日小记</h3>
          <textarea
            ...
            placeholder="今天在固定时辰回到神面前，有什么领受或挣扎？"
          />
        </article>
      </div>
      ...
      <div className="sf-card">
        ...
        ) : <p className="sf-empty">还没有历史记录。完成今日时辰并保存后，这里会显示连续记录。</p>}
      </div>
    </section>
  )
}
