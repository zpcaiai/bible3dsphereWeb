    ...
    <div className="cc-field">
      ...
      {(items || []).map((it, i) => (
        <div className="cc-list-item" key={`${it}-${i}`}>
          ...
          <button type="button" aria-label="删除" onClick={() => onChange(items.filter((_, j) => j !== i))}>×</button>
        </div>
      ))}
      <div className="cc-list-item">
        ...
        <button type="button" aria-label="新增" onClick={add} style={{ color: '#34c759' }}>＋</button>
      </div>
    </div>
  )
}
...
export default function SafetyPlanEditor({ plan, regionCode = 'TW', onSave, syncing }) {
  ...
  return (
    <div className="cc-card">
      <h3>我的安全计划</h3>
      <p className="cc-muted">在平静的时候写好它，危机时它会替你记得该做什么。</p>

      <ListEditor label="我的危机警讯（提前察觉）" items={draft.warningSigns} onChange={(v) => patch('warningSigns', v)} placeholder="例如：连续失眠 / 想消失" />
      <ListEditor label="我可以先做的 5 分钟动作" items={draft.internalCopingStrategies} onChange={(v) => patch('internalCopingStrategies', v)} placeholder="例如：打开灯、喝水" />
      <ListEditor label="我的安全联系人（至少一位真人）" items={draft.safePeople} onChange={(v) => patch('safePeople', v)} placeholder="姓名 + 联系方式" />
      <ListEditor label="我可以去的安全地点" items={draft.safePlaces} onChange={(v) => patch('safePlaces', v)} placeholder="例如：客厅 / 楼下便利店" />
      <ListEditor label="减少危险物品接触" items={draft.meansRestrictionSteps} onChange={(v) => patch('meansRestrictionSteps', v)} placeholder="例如：把药交给家人保管" />
