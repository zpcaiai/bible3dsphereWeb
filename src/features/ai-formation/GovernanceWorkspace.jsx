...
export default function GovernanceWorkspace({ releaseGates, user }) {
  ...
  const [review, setReview] = useState({ reviewer_role: 'theology_reviewer', decision: 'approve', reason: '' })
  ...
  const openVersion = async (item) => {
    ...
    try {
      ...
      setReview({ reviewer_role: role, decision: 'approve', reason: '' })
  ...
  const submitReview = async (event) => {
    ...
    try {
      await reviewAiFormationContent(selected.content.id, selected.content.version, {
        ...
        reason_codes: [review.reason.trim() || 'AUTHORIZED_HUMAN_REVIEW'], note: '',
      })
    ...
    <div className="aif-stack">
      ...
      {selected && <section className="aif-card aif-review-detail">
        ...
        <form className="aif-evidence-form" onSubmit={submitReview}>
          <label>reviewer_role<select value={review.reviewer_role} onChange={(event) => setReview({ ...review, reviewer_role: event.target.value })}>{selected.content.required_reviews_json.map((role) => <option key={role}>{role}</option>)}</select></label>
          ...
          <label>reason_code<input value={review.reason} onChange={(event) => setReview({ ...review, reason: event.target.value })} /></label>
